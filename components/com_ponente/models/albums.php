...
if (empty($list['direction']))
{
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(#__ponente_grupo_2388112.nombre LIKE ' . $search . '  OR  a.nombre LIKE ' . $search . ' )');
			}
		}
	...
	public function getItems()
	{
		...
		{
			if (isset($item->grupo) && $item->grupo != '')
			{
				if (is_object($item->grupo))
				{
					$item->grupo = \Joomla\Utilities\ArrayHelper::fromObject($item->grupo);
				}

				$values = (is_array($item->grupo)) ? $item->grupo : explode(',', $item->grupo);
				$textValue = array();
				...
				$item->grupo = !empty($textValue) ? implode(', ', $textValue) : $item->grupo;
			}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_PONENTE_SEARCH_FILTER_DATE_FORMAT"), "warning");
